...
class BallTracker:
    ...
    def interpolate_ball_positons(self, ball_positions):
        ...
        ball_positions = [{1:x} for x in df_ball_positions.to_numpy().tolist()] # créée une liste de dictionnaires ou 1 c'est le track id et x la bounding box

        return ball_positions

    def get_ball_shot_frames(self, ball_positions, yolo = True):
    ...
    def detect_frames(self, frames, read_from_stub=False, stub_path=None):
        ...
        return ball_detections  # renvoit la liste des dictionnaires de joueurs détectés

    def detect_frame(self, frame):
        results = self.model.predict(frame,conf=0.2)[0]  # Persist dis que l'objet est le même entre les frames
        if results.boxes is None:
            return {}  # No ball detected, return an empty dictionary
        ball_dict = {}
        for box in results.boxes:
            result = box.xyxy.tolist()[0]
            ball_dict[1] = result # ajoute l'id du joueur associé aux coordonnées du joueur dans le dictionnaire
        return ball_dict
